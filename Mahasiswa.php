		...
		if(mysql_num_rows($query) == 0){	//ini artinya jika data hasil query di atas kosong
			...
			while($data = mysql_fetch_assoc($query)){	//perulangan while dg membuat variabel $data yang akan mengambil data di database
				...
				echo '<tr>';
					...
					echo '<td><a href="editMhs.php?id='.$data['NIM'].'">Edit</a> / <a href="hapusMhs.php?id='.$data['NIM'].'" onclick="return confirm(\'Apakah Anda Yakin ?\')">Hapus</a></td>';	//menampilkan link edit dan hapus dimana tiap link terdapat GET id -> ?id=siswa_id
				echo '</tr>';
